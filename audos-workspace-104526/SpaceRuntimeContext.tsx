...
import {
  getDelegationChatRequestExtras,
  getTenantDelegationScope,
  hasTenantDelegationAuth,
...
export function SpaceRuntimeProvider({ 
  ...
  const [sessionId, setSessionIdRaw] = useState<string | undefined>(() => {
    ...
    try {
      const stored = localStorage.getItem(scopedSpaceSessionStorageKey(spaceId));
      if (stored) {
  ...
  useEffect(() => {
    ...
    const autoCreateSession = async () => {
      try {
        ...
        const sessionKey = scopedSpaceSessionStorageKey(spaceId);
        const session = {
  ...
  // Product Run / tenant handoffs can arrive without a workspace session. Turn
  // the delegated credentials into a normal workspace session before rendering
  // the gate so the rest of the app uses the standard session contract.
  useEffect(() => {
    if (mode !== 'customer' || !hasTenantDelegationAuth()) return;
    const delegation = getTenantDelegationScope();
    const extras = getDelegationChatRequestExtras();
    if (!extras) return;

    const injectedId = delegation?.workspaceSessionId;
    if (injectedId?.startsWith('wses_')) {
      setSessionId(injectedId);
      setIsBootstrappingSession(false);
      return;
    }
    ...
    let cancelled = false;
    const bootstrapDelegationSession = async () => {
      setIsBootstrappingSession(true);
      try {
        const response = await fetch(`/api/space/${spaceId}/delegation-session-bootstrap`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            delegationToken: extras.delegationToken,
            taSessionId: extras.taSessionId,
          }),
        });
        if (!response.ok) throw new Error(`delegation bootstrap failed: ${response.status}`);
        const data = await response.json();
        const effectiveSessionId = data.workspaceSessionId || data.sessionId;
        if (!effectiveSessionId || cancelled) return;
        localStorage.setItem(
          scopedSpaceSessionStorageKey(spaceId),
          JSON.stringify({
            id: effectiveSessionId,
            workspaceSessionId: effectiveSessionId,
            email: data.email || delegation?.email || null,
            timestamp: Date.now(),
            verified: true,
            metadata: { source: 'tenant_delegation' },
          }),
        );
        setSessionId(effectiveSessionId);
      } catch (error) {
        console.warn('[SpaceRuntime] Delegation session bootstrap failed:', error);
      } finally {
        if (!cancelled) setIsBootstrappingSession(false);
      }
    };
    bootstrapDelegationSession();
    return () => {
      cancelled = true;
    };
