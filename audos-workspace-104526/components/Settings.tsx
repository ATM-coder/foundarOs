...
// STRIPE TEST MODE — keep in sync with STRIPE_TEST_MODE in
// apps/GapRadar/App.tsx. The platform payments API (/api/payments/*) runs
// LIVE Stripe keys server-side (no keys exist in this codebase to swap), so
// while this is true the subscribe button is disabled instead of opening a
// live-mode checkout that would charge a real card.
...
export default function Settings({ spaceId }: SettingsProps) {
  ...
  useEffect(() => {
    ...
    // Test mode intentionally avoids the live payment configuration endpoint.
    // This prevents a missing optional config from producing a console error and
    // guarantees Settings cannot initialize any live checkout state.
    if (!STRIPE_TEST_MODE) {
      loadPaymentConfig();
    }
  ...
  const handleSubscribe = async () => {
    if (STRIPE_TEST_MODE) {
      setError('Payments are in test mode — checkout is temporarily disabled and no cards are charged.');
      return;
    }
    setSubscribing(true);
    try {
                ...
                {showPaymentCTA && (
                  <div className="space-y-3">
                    ...
                    <button
                      ...
                      disabled={subscribing || STRIPE_TEST_MODE}
                    ...
                    <p className="text-center text-xs text-gray-500">
                      {STRIPE_TEST_MODE
                        ? 'Checkout is disabled while test mode is active — no cards are charged.'
